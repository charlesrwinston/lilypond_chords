...
(define-public (get-modifications-from-semantics semantics-list)
  (define (not-root-entry? pair)
    (not (eq? (car pair) 'root)))
  (filter not-root-entry? semantics-list))
(define-public (get-root-from-semantics semantics-list)
  (assoc-ref 'root semantics-list))
    ...
    (define (get-semantics-event music)
      (let* ((return '())
             ...
             (e (if (ly:music? music)
                    ...
                    '())))
        (cond ((ly:music? e)
               (set! return (get-semantics-event e)))
              ((not (null? es))
               ;; need to set return to the car of the list in order for it
               ;; to be music expression. The list can only be one element
               ;; anyway, the chord semantics element.
               (set! return (car (flatten-list (map get-semantics-event es)))))
              ((memq 'chord-semantics-event (ly:music-property music 'types))
               (set! return music)))
        return))
    (let* ((semantics-event (get-semantics-event chord))
           (semantics-list (ly:music-property semantics-event 'chord-semantics)))
      semantics-list))
  (list (cons (get-modifications-from-semantics (get-semantics-list chord))
              markup)))
